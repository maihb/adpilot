...
> **状态：14 个里程碑里的 D1–D14，功能范围已走完。** 整条链是：导入 CSV → 原始
> 快照 → 归一化日指标 → 规则巡检 → 告警推送 → **日报**（数字由代码算、人话由 LLM
> 起草、人改完才能发）→ 客户端只看得到已发布的那份。**还欠着两处界面**：日报的
> 编辑发布屏和客户端的日报屏 —— 后端接口都在了，但运营现在只能直接调接口。
...
时区，外加每个账户一条投放操作记录和一份**已发布的昨日日报**。它**只添不改**，
重复跑安全；`ENVIRONMENT=prod` 时直接拒绝执行。
