...
| **两个日报开关**（D18） | `auto_report` 要不要每天自动出日报（🔴 **它是省钱的闸门** —— 不看日报的账户没必要每天烧一次 LLM 调用），`report_delay_hours` 日切后等几小时再生成（0–72，默认 2）。两个都能 PATCH。⚠️ `auto_report` 和 `is_active` 是**两件事**：停投的账户仍然可能要日报（复盘那几天怎么停的） |
