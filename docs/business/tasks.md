...
| 规则 | 一句话 |
|---|---|
| 现在有四个任务 | `adpilot.normalize_account`（快照 → 日指标）、`adpilot.sweep_alerts`（每小时巡检告警）、`adpilot.generate_due_reports`（定时日报）、`adpilot.fetch_due_accounts`（自动拉取） |
| **加任务要改两处** | 任务体一个文件，外加 `tasks/app.py` 的 `TASK_MODULES` 补一行 —— 漏了的话 worker 认不出它，而报错在另一个进程的日志里 |
...
- **三个定时任务，都是每小时一次，而且这条链是有方向的**：自动拉取在第 40 分
  （D19）→ 告警巡检在整点（D8，用刚拉到的数判告警）→ 定时日报在第 20 分（D18，
  引用刚开的告警）。**错开是刻意的**：任意两个撞在同一分钟，中间那一环就可能读到
  上一环还没写完的东西，而谁先跑取决于 worker 取消息的顺序。有测试盯着这三个数
  两两不相等。
- ⚠️ **日报那个任务会花钱**，这是它和巡检最大的不同（巡检跑一万次也只是几条
  SQL）。密集排期靠的是判定便宜：绝大多数轮次扫出来是空的，因为那天已经有日报
  了。它另外两条护栏 —— 每份各自一个事务、撞额度就整轮中断 —— 见
  [定时日报设计](../design/2026-08-21-scheduled-reports.md)第七节。
