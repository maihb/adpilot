...
| 规则 | 一句话 |
|---|---|
| 现在有两个任务 | `adpilot.normalize_account`（快照 → 日指标）和 `adpilot.sweep_alerts`（每小时巡检告警） |
...
- **两个定时任务，都是每小时一次。** 告警巡检在整点（D8），定时日报在第 20 分
  （D18）—— **错开是刻意的**：日报要引用「当时开着的告警」，而巡检正是产出那些
  告警的，同一分钟投出来时谁先跑取决于 worker 取消息的顺序。有测试盯着这两个
  数不相等。「每天自动拉数」仍然没有 —— MVP 不接 Ads API，没有可拉的东西。
