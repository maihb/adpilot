...
| **依赖不许悄悄腐化** | [`.github/dependabot.yml`](.github/dependabot.yml)：action、两个前端的 npm、`uv.lock` 各自月度合成一个 PR。⚠️ **这条不是门禁** —— 它拦不住任何东西，只保证「过时」会变成一个要处理的 PR，而不是一条没人看的 CI annotation（`Node.js 20 is deprecated` 就那样被忽略了一整段时间） |
