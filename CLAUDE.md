...
| **依赖不许悄悄腐化** | [`.github/dependabot.yml`](.github/dependabot.yml)：action、**admin 的** npm、`uv.lock` 各自月度合成一个 PR。⚠️ **client 不在里面**：它的整条工具链被 uni-app 的**精确** peer 钉死（`vite: '5.2.8'`）、`@dcloudio/*` 又必须同一个日期构建号，是个**必须整体替换**的集合，而 Dependabot 是逐包升级 —— 连撞两个红 PR 撞出来的结论，升级 client 要人整体来。⚠️ 这条**也不是门禁**，它拦不住任何东西，只保证「过时」会变成一个要处理的 PR，而不是一条没人看的 CI annotation |
...
| **枚举的中文名不许和后端漂移** | `tests/test_frontend_source.py` 四条：告警类型、日报状态各一条（长在页面里），外加 `admin/src/api/enums.ts` 那个**枚举镜像模块**双向对齐 `ActionKind` / `MetricLevel` / `services/product.py` 的 `SALES_*`。第四条更要紧 —— **它禁止页面里再长出新的映射**，因为散着的拷贝不在任何门禁的视野里，而对不上时只会安静地把一个英文标识显示给人 |
| **页面里不许把字符串转成数字** | `tests/test_frontend_source.py` 扫源码：后端的金额和比率下发的**永远是字符串**，而 `Number(null) === 0` —— 一个 `Number()` 就把「算不出来」显示成「还能撑 0 天」，于是每个暂停投放的账户都在客户屏幕上着火。转换只许发生在两个前端各自的 `src/utils/` 里。同一个文件还盯着网络调用只走唯一出口（客户端 `uni.request`、后台 `fetch`）、以及模板里不许写 markdown 的 `**`（⚠️ 多行 HTML 注释里也不许 —— 只有第一行匹配得上注释形状，后面几行照样被扫）|
| **三个「跑起来才看得见」的显示坑** | `tests/test_frontend_source.py` 三条形状门禁 + `admin/src/api/request.test.ts`：①页面不许自己判错误类型（`instanceof NeedsRedo` 那种三元会把后端那句**能指导操作**的话吞成「操作失败」，一律走 `reason()`）；②日报详情页不许渲染操作时刻（`performed_at` 按手机时区渲染会**跨天** —— 08-19 的日报里出现 08-20 的操作）；③环比脚注的判据必须是「有没有算出环比」而不是「有没有对照期」（对照期存在但值为 0 时同样算不出，那时客户看不到任何解释）。**三条都是实跑逮到的**，静态检查一个字都不会说 |
...
| **工作目录不许乱跳** | 守卫拦下 `cd` / `pushd` / `popd`，只放行「回仓库根」这一条逃生口。访问项目内的文件一律用**相对路径**（pwd 恒定是仓库根），要在别处执行就用 `git -C` / `make -C` / `docker compose -f` 这类目录参数。理由：cd 是会话级的且**没有回声**，代价不在当前这条命令、在后面每一条 |
| **`git -C` 按目标判，不再一刀切** | 目标在仓库内 + 子命令只读才自动放行，指向仓库外的仍逐次确认（`git -C ../别的仓库 push` 会绕过 `Bash(git push:*)` 那条 deny —— 前缀匹配对不上）。**这条成立的前提是上一行**：pwd 被钉死，`-C docs` 才解释得出唯一目标。两条连在一起，改一条要看另一条 |
